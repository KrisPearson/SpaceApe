...
UNetworkGameInstance::UNetworkGameInstance() {
	...
	//OnSessionInviteReceivedDelegate = FOnSessionInviteReceivedDelegate::CreateUObject(this, &UNetworkGameInstance::OnSessionInviteReceived);
...
//////////////////////////////////////////////////////////
			GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("SessionSettings. map name = %s"), *LobbyMapName.ToString() ));
			...
			UE_LOG(LogTemp, Warning, TEXT("OnCreateSessionCompleteDelegateHandle"));
...
void UNetworkGameInstance::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful) {
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT(" Session Not Valid %s, %d"), *SessionName.ToString(), bWasSuccessful));
		}

	}
	else {
		GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Online Sub = null %s, %d"), *SessionName.ToString(), bWasSuccessful));
	}
}
...
/*void UNetworkGameInstance::OnSessionInviteReceived(const FUniqueNetId & InvitedPlayer, const FUniqueNetId & InvitingPlayer, const FString & AppID, const FOnlineSessionSearchResult & TheSessionInvitedTo)
{
...
UTexture2D* UNetworkGameInstance::GetSteamAvatar(const FBPUniqueNetId UniqueNetId) {
	if (UniqueNetId.IsValid())
	{
		...
		if (Width > 0 && Height > 0)
		{
			...
			Avatar->PlatformData->PackedData = 1;
